...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "wrk01" do |wrk01|
    wrk01.vm.box = "centos/7"
    wrk01.vm.hostname = 'wrk01'
    wrk01.vm.box_url = "centos/7"
  ...
  config.vm.define "wrk02" do |wrk02|
    wrk02.vm.box = "centos/7"
    wrk02.vm.hostname = 'wrk02'
    wrk02.vm.box_url = "centos/7"
  ...
  config.vm.define "wrk03" do |wrk03|
    wrk03.vm.box = "centos/7"
    wrk03.vm.hostname = 'wrk03'
    wrk03.vm.box_url = "centos/7"
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    sudo sh -c 'echo "192.168.1.101 wrk01" >> /etc/hosts'
    sudo sh -c 'echo "192.168.1.102 wrk02" >> /etc/hosts'
    sudo sh -c 'echo "192.168.1.103 wrk03" >> /etc/hosts'
